 ...
 */
export function defaultSourcePresetId() {
    return 'standard';
}
/**
 ...
 */
export function stripPresetRows(composition, ids) {
 ...
 */
export function findDshPresetRoot(cwd) {
    ...
    for (const root of candidates) {
        if (existsSync(join(root, 'config', 'agent-presets')))
            return root;
    }
    return undefined;
}
...
export function parseArgs(argv) {
    const out = { id: 'abap-adt', from: undefined, name: 'ABAP Development', force: false, dryRun: false, help: false };
        ...
        switch (arg) {
            ...
            case '--from':
                ...
                if (out.from === undefined || !/^[a-z0-9][a-z0-9-]*$/.test(out.from)) {
                    ...
                    throw new Error(`--from needs a shipped preset id matching [a-z0-9][a-z0-9-]* (got '${out.from ?? 'nothing'}')`);
                }
                break;
            case '--name':
                out.name = argv[++i] ?? '';
...
export function main(argv) {
    ...
    try {
        ...
        process.stderr.write(`abap-adt-preset: ${error.message}\n`);
        return 1;
    }
    if (args.help) {
    ...
    if (!isDirectory(sourceDir) || !existsSync(join(sourceDir, 'agent.cordis.yml'))) {
        process.stderr.write(`abap-adt-preset: source preset '${from}' not found under ${join(root, 'config', 'agent-presets')}\n`);
        return 1;
    }
    ...
    if (args.dryRun) {
        process.stdout.write(`would copy  ${sourceDir} -> ${presetDir}\n` +
