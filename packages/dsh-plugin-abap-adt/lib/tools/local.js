 ...
 */
export function toAbaplintName(fileName, source) {
    ...
    if (lower.endsWith('.abap')) {
        ...
        if (typed && TYPED_SUFFIXES.has(typed))
            return lower;
        ...
        if (/^INTERFACE\b/.test(head))
            return `${base}.intf.abap`;
        if (/^FUNCTION-POOL\b/.test(head))
            return `${base}.funcs.abap`;
        if (/^(REPORT|PROGRAM)\b/.test(head))
            return `${base}.prog.abap`;
    }
    return undefined;
}
async function collectSources(dir, fs, maxFiles) {
    const files = [];
    let skipped = 0;
    let truncated = false;
    const visit = async (absPath) => {
...
async function resolveConfig(fs, dir, configPath) {
    if (configPath)
        return { config: new Config(await fs.readFile(configPath)), source: 'explicit' };
    try {
...
export async function runLocalCheck(dir, options, fs) {
    ...
    const filtered = all.filter((issue) => allowed.has(issue.getSeverity()));
    return {
        dir,
...
function renderLocalCheck(args, value) {
...
export function localTools(_deps, ctx) {
    return [
        defineTool({
            name: 'adt_local_check',
            description: 'Run abaplint (offline static analysis) over a directory of ABAP sources (.abap). Pairs with ' +
            ...
            parameters: {
                ...
                configPath: {
                    type: 'string',
                    description: 'Absolute path to an .abaplint.json. Default: <dir>/.abaplint.json if present, else abaplint built-in defaults.',
                },
                severity: {
                    type: 'string',
