...
export function lockTools(deps) {
    ...
    const unlockAll = defineTool({
        name: 'adt_unlock_all',
        description: 'Release residual ABAP edit locks held by this user. ADT locks can survive a crashed or interrupted tool ' +
        ...
        parameters: {
            objects: {
                type: 'array',
                description: 'Optional explicit objects to unlock. Each entry: {objectUri} or {name, type}. ' +
        ...
        execute: async (args, exec) => {
            ...
            for (const ref of explicitRefs)
                candidates.set(ref.uri, { uri: ref.uri, name: ref.name });
