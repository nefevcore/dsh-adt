...
export function replaceSourceBlock(source, startText, endText, replacement) {
    ...
    if (startHits.length === 0)
        throw new Error(`adt_edit_object: start line "${startText}" not found`);
    ...
    if (endHits.length > 1) {
        ...
        throw new Error(`adt_edit_object: end marker "${endText}" is ambiguous (${endHits.length} matches after the start: ${candidates}); ` +
    ...
    const editSource = defineTool({
        ...
        parameters: {
            ...
            start: {
                ...
                type: 'string',
                description: 'Last line of the block, e.g. "ENDMETHOD.". Optional: auto-derived from the block type.',
            },
        ...
        execute: async (args, exec) => {
            ...
            if (!endText) {
                throw new Error(`adt_edit_object: cannot auto-derive the end line for "${startText}" — provide \`end\` explicitly`);
            }
