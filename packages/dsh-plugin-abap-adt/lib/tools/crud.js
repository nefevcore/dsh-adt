...
import { crudCell, crudObjectTypes, crudUnsupportedMessage, crudVerbsFor, CRUD_VERBS, } from '../crudmatrix.js';
import { DESTINATION_PARAM, optStr, text } from './common.js';
export function crudTools(deps, tools) {
    void deps; // (owners carry their own deps; the facade only routes)
    const owner = (name) => {
        const tool = tools.get(name);
        if (!tool) {
            throw new Error(`adt_crud: the matrix routes to '${name}' but that tool is not registered — ` +
    ...
    function assertVerbArgs(verb, args) {
        const name = optStr(args.name) ?? optStr(args.objectUri);
        if (!name)
            throw new Error(`adt_crud: ${verb} requires the object reference (name or objectUri)`);
        if (verb === 'create') {
            if (!optStr(args.description))
                throw new Error('adt_crud: create requires `description`');
            if (!optStr(args.packageName))
                throw new Error('adt_crud: create requires `packageName` (use $TMP for local objects)');
        }
        if (verb === 'update') {
            const hasSource = optStr(args.source) !== undefined || optStr(args.sourceFile) !== undefined;
            const hasChanges = args.changes !== undefined || args.properties !== undefined || args.messages !== undefined ||
                args.fixedValues !== undefined || args.labels !== undefined || optStr(args.description) !== undefined;
            if (!hasSource && !hasChanges) {
                throw new Error('adt_crud: update requires either `source`/`sourceFile` (source objects) or the structured ' +
    ...
    return [
        defineTool({
            name: 'adt_crud',
            description: 'Create / read / update / delete ABAP development objects through ONE verb-routed tool: pass `verb` + ' +
            ...
            execute: async (args, exec) => {
                ...
                if (!verb) {
                    return { matrix: renderMatrixForCard(), routedTool: 'adt_crud (status card)' };
                }
                ...
                if (!type)
                    throw new Error(`adt_crud: ${verb} requires \`type\` (${crudObjectTypes().join(', ')})`);
                const cell = crudCell(verb, type);
                if (!cell)
                    throw new Error(crudUnsupportedMessage(verb, type));
                ...
                const result = (await tool.execute(forwarded, exec));
                return { ...result, routedTool: cell.tool, verb, type: type.toUpperCase() };
            },
        }),
    ];
}
/** The status-card matrix (compact render of crudmatrix.ts). */
function renderMatrixForCard() {
