...
function renderStructure(data) {
    ...
    if (data.packageName)
        lines.push(`package: ${String(data.packageName)}`);
    if (data.kind === 'MSAG') {
        const messages = data.messages ?? [];
        lines.push(`messages (${messages.length}):`);
        for (const m of messages)
            lines.push(`  ${m.number}: ${m.text}`);
    }
    else {
        const props = data.properties ?? {};
        lines.push(`properties: ${Object.entries(props).map(([k, v]) => `${k}=${v}`).join(', ') || '(none)'}`);
        if (data.kind === 'DOMA') {
            const fixed = data.fixedValues ?? [];
...
function structureToOutput(data) {
    const out = {
        kind: data.kind,
        name: data.name,
    ...
    if (data.kind === 'MSAG') {
        out.messages = data.messages.map((m) => {
            const message = { number: m.number, text: m.text };
            if (m.selfExplanatory !== undefined)
                message.selfExplanatory = m.selfExplanatory;
            return message;
        });
    }
        ...
        out.fixedValues = data.fixedValues.map((f) => {
            const fixed = { low: f.low };
            if (f.high !== undefined)
                fixed.high = f.high;
            if (f.description !== undefined)
                fixed.description = f.description;
            return fixed;
        });
    }
...
export function structureTools(deps) {
    const { registry } = deps;
    const readStructure = defineTool({
        name: 'adt_read_structure',
        description: 'Read the STRUCTURED metadata of a DDIC object as typed JSON — use this (not adt_read_object) for ' +
        ...
        execute: async (args, exec) => {
            ...
            try {
                ...
                if (error instanceof AdtError && (error.status === 404 || error.status === 405 || error.status === 406)) {
                    throw new Error(`adt_read_structure: ${kind} editor not available for ${ref.name} on destination ` +
    ...
    const writeStructure = defineTool({
        ...
        description: 'Change the STRUCTURED metadata of a DDIC object (message classes/domains/data elements/table ' +
        ...
        output: {
            ...
            render: (_args, value) => text([`${value.name} (${value.kind}) updated${value.transport ? ` [transport ${value.transport}]` : ''}:`, `  changed: ${value.changed.join(', ') || '(nothing)'}`, '', ...renderStructure(value.data)].join('\n')),
        },
        timeoutMs: 180_000,
        execute: async (args, exec) => {
            const entry = registry.require(destinationOf(args));
