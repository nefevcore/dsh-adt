...
export function tableGlobToRegExp(pattern) {
...
const COMPILED = CATALOG.map((entry) => {
    ...
    for (const raw of entry.names) {
        if (raw.includes('*'))
            patterns.push(tableGlobToRegExp(raw));
        else
            exact.add(raw.toUpperCase());
    }
 ...
 */
export function findBlockedTable(table, profile, extra = []) {
    if (profile === 'off')
        return null;
    ...
    for (const entry of COMPILED) {
        if (TIER_ORDER[entry.tier] > maxTier)
            continue;
