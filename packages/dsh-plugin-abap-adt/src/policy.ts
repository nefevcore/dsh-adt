...
export class AdtPolicy {
  ...
  static resolve(config: PolicyInputs, env: NodeJS.ProcessEnv = process.env): AdtPolicy {
    ...
    if (config.enableTransports !== undefined) sources.enableTransports = 'config';
    else if (enableTransports !== undefined && env[POLICY_ENV.enableTransports] !== undefined) sources.enableTransports = 'env';

    const allowTransportableEdits =
      config.allowTransportableEdits ?? parseEnvBoolean(env[POLICY_ENV.allowTransportableEdits]);
    if (config.allowTransportableEdits !== undefined) sources.allowTransportableEdits = 'config';
    ...
    if (config.allowedTransports !== undefined) sources.allowedTransports = 'config';
    else if (env[POLICY_ENV.allowedTransports] !== undefined) sources.allowedTransports = 'env';

    const allowedPackagesRaw = config.allowedPackages ?? env[POLICY_ENV.allowedPackages];
    if (config.allowedPackages !== undefined) sources.allowedPackages = 'config';
    else if (env[POLICY_ENV.allowedPackages] !== undefined) sources.allowedPackages = 'env';

    const allowExecution = config.allowExecution ?? parseEnvBoolean(env[POLICY_ENV.allowExecution]);
    if (config.allowExecution !== undefined) sources.allowExecution = 'config';
    else if (allowExecution !== undefined && env[POLICY_ENV.allowExecution] !== undefined) sources.allowExecution = 'env';

    const allowBatchWrites = config.allowBatchWrites ?? parseEnvBoolean(env[POLICY_ENV.allowBatchWrites]);
    if (config.allowBatchWrites !== undefined) sources.allowBatchWrites = 'config';
