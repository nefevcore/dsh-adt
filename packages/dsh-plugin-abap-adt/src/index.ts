 ...
 */
const DSH_HOST_PROFILE: HostProfile = {
  id: 'dsh',
  label: 'DSH',
...
async function apply(ctx: Context, config: PluginConfig): Promise<() => Promise<void>> {
  ...
  const dshLogger = (ctx as unknown as ToolHost).logger;
  const host: ToolHost = {
    get: (serviceName: string) => (serviceName === 'host' ? DSH_HOST_PROFILE : ctx.get(serviceName)),
