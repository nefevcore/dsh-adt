 ...
 */
export function candidateAdtUrls(host: string, sysnr: string | undefined): string[] {
  const urls = sysnr
    ? [
...
function failureReason(error: Error): string {
  ...
  if (code === 'ENOTFOUND') return 'host not found (DNS)';
  if (code === 'ETIMEDOUT' || /timeout/i.test(error.message)) return 'no response (timeout)';
...
function classifyStatus(url: string, status: number): UrlProbe {
  const adtLikely = (status >= 200 && status < 400) || status === 401 || status === 403;
  let detail: string;
  if (status === 401) detail = 'HTTP 401 — endpoint responded, authentication required (expected for ADT without credentials)';
  else if (status === 403) detail = 'HTTP 403 — endpoint responded but access is blocked (SICF service or ICM filter?)';
  else if (status >= 200 && status < 300) detail = `HTTP ${status} — endpoint responded without challenge`;
  else if (status >= 300 && status < 400) detail = `HTTP ${status} — redirect (endpoint responded)`;
  else if (status === 404) detail = `HTTP 404 — port answers HTTP but no ADT service at ${ADT_PROBE_PATH}`;
 ...
 */
export function probeUrl(url: string, options: ProbeOptions = {}): Promise<UrlProbe> {
  ...
  return new Promise((resolve) => {
    ...
    let settled = false;
    const finish = (probe: UrlProbe): void => {
      if (settled) return;
      settled = true;
      resolve(probe);
    };
    const request = (target.protocol === 'http:' ? httpGet : httpsGet)(
      target,
      {
 ...
 */
export function pickVerifiedProbe(probes: UrlProbe[]): UrlProbe | undefined {
  return (
    probes.find((p) => p.reachable && p.adtLikely) ?? probes.find((p) => p.reachable)
  );
}

/** 'https://h:44301 → connection refused; https://h → no response (timeout); …' */
export function summarizeProbes(probes: UrlProbe[]): string {
 ...
 */
export function unreachableGuidance(router?: string): string {
  const causes = [
