...
export class DebuggerManager {
  ...
  constructor(private readonly registry: AdtRegistry) {}
  ...
  ): Promise<AdtDebugListenResult & { terminalId: string; ideId: string }> {
   ...
   */
  async detach(entry: RegistryDestination, signal?: AbortSignal): Promise<{ detached: boolean; note?: string }> {
    const record = this.sessions.get(entry.config.name);
    this.dropSession(entry);
    if (!record) return { detached: false, note: 'no session registered for this destination' };
    try {
      ...
      return { detached: false, note: `backend detach answered: ${(error as Error).message}` };
    }
  }
  ...
  describe(
    entry: RegistryDestination,
  ): { registered: boolean; terminalId?: string; user?: string; lastListen?: DebugSessionRecord['lastListen'] } {
    const record = this.sessions.get(entry.config.name);
...
function defaultDebugUser(entry: RegistryDestination): string {
  const auth = (entry.client.destination as { auth?: { type?: string; username?: string } }).auth;
