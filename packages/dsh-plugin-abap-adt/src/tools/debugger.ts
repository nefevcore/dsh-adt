...
import type { AdtDebugBreakpoint, AdtDebugStepResult } from '@nefevcore/abap-adt-protocol';
import {
  DESTINATION_PARAM,
  destinationOf,
  optStr,
...
export function debuggerTools(deps: ToolDeps) {
  ...
  return [
    defineTool({
      ...
      description:
        'Debug ABAP code live over the standard ADT debugger (SE37-style new debugger, /sap/bc/adt/debugger): ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          if (value.debuggee) {
            const d = value.debuggee as { program?: string; line?: number; user?: string; id?: string };
            lines.push(
              `BREAKPOINT HIT — stopped in ${d.program ?? '?'}${d.line ? ` line ${d.line}` : ''}` +
          ...
          if (value.timedOut) lines.push('no breakpoint hit within the wait window (listener stays registered)');
          if (value.conflict) lines.push(`listener conflict: ${value.conflict}`);
          if (value.session) {
            const s = value.session as { registered?: boolean; user?: string; lastListen?: { at: string; hit: boolean; program?: string } };
            lines.push(
              s.registered
          ...
          if (value.detached !== undefined) lines.push(value.detached ? 'listener detached — a fresh listen needs fresh state' : (value.note ?? ''));
      ...
      execute: async (args, exec) => {
        ...
        if (action === 'status') {
          ...
          let backendListeners: string | undefined;
          try {
            backendListeners = (await entry.client.debuggerListenerStatus({ user, signal: exec.signal })).rawXml;
          } catch (error) {
            backendListeners = `backend listener query failed: ${(error as Error).message}`;
          }
    ...
    defineTool({
      ...
      description:
        'Set or delete a debugger LINE breakpoint in ABAP code (external scope — it fires for the target user when ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          if (value.action === 'set') {
            const bps = (value.breakpoints ?? []) as AdtDebugBreakpoint[];
            return text(
              `breakpoint(s) set on ${value.destination}:\n` +
      ...
      execute: async (args, exec) => {
        ...
        if (action === 'set') {
          ...
          if (typeof line !== 'number' || line < 1) throw new Error('adt_debug_breakpoint: `line` (1-based) is required for set');
          let uri = optStr(args.objectUri);
          if (!uri) {
            const name = optStr(args.name);
            if (!name) throw new Error('adt_debug_breakpoint: provide objectUri or name(+type) for set');
        ...
        if (action === 'delete') {
          ...
          if (!id) throw new Error('adt_debug_breakpoint: `id` (from set) is required for delete');
          const record = manager.sessionFor(entry, user);
    ...
    defineTool({
      name: 'adt_debug_step',
      description:
        'Step the stopped ABAP debuggee: stepInto (F5), stepOver (F6), stepReturn (F7), stepContinue (F8 — run free ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const r = (value.result ?? {}) as Partial<AdtDebugStepResult>;
          const lines = [
          ...
          if (r.reachedBreakpoints?.length) lines.push(`reached breakpoints: ${r.reachedBreakpoints.map((b) => b.id).join(', ')}`);
          return text(lines.join('\n'));
        },
      },
      execute: async (args, exec) => {
        ...
        const result = await entry.client.debuggerStep({
          step: step as keyof typeof STEP_NAMES & 'stepInto',
          signal: exec.signal,
        });
        return {
          step,
    ...
    defineTool({
      ...
      description:
        'Inspect the stopped ABAP debuggee: `variables` reads the values of named variables (LV_COUNT, WA_MARA, ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          if (value.action === 'variables') {
            const vars = (value.variables ?? []) as unknown as Array<{ name: string; value?: string; declaredTypeName?: string; readOnly?: boolean }>;
            if (vars.length === 0) return text('no variables returned (not stopped, or names not found)');
            return text(
              vars
          ...
          const stack = (value.stack ?? {}) as unknown as {
            entries?: Array<{ stackPosition: number; programName?: string; includeName?: string; line?: number; eventType?: string; eventName?: string }>;
            unavailable?: boolean;
            note?: string;
            cursorIndex?: number;
          };
          if (stack.unavailable) return text(`stack unavailable: ${stack.note ?? 'backend lacks /debugger/stack'}`);
          return text(
            `call stack (cursor at frame ${stack.cursorIndex ?? 0}):\n` +
              (stack.entries ?? [])
      ...
      execute: async (args, exec) => {
        ...
        if (action === 'variables') {
          ...
          if (names.length === 0) throw new Error('adt_debug_inspect: `variables` (non-empty name list) is required');
    ...
    defineTool({
      ...
      description:
        'Change the VALUE of one variable in the stopped ABAP debuggee (watchpoint-style value override). ' +
      ...
      execute: async (args, exec) => {
        ...
        if (!name || value === undefined) throw new Error('adt_debug_set_variable: `name` and `value` are required');
