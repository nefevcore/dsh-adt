...
export function gateTools(deps: ToolDeps) {
  ...
  return [
    defineTool({
      ...
      description:
        'Verify a package or object set before release: runs syntax check + ABAP Unit + ATC over a DEVC ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const lines = [
            ...
            ...value.stages.map((s) =>
              `- ${s.stage}: ${s.skipped ? 'SKIPPED' : s.pass ? 'PASS' : 'FAIL'} — ${s.summary}`,
            ),
          ];
      ...
      execute: async (args, exec) => {
        ...
        const runStage = async (stage: GateStage, fn: () => Promise<GateStageResult>): Promise<void> => {
          try {
