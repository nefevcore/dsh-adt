 ...
 */
import { defineTool, type JsonValue, type ToolHost } from '../tooldef.js';
...
export function readTools(deps: ToolDeps, ctx?: ToolHost) {
  const { registry } = deps;

  const readObject = defineTool({
    name: 'adt_read_object',
    description:
      'Read the source code and metadata of an ABAP development object — classes (CLAS), interfaces (INTF), ' +
    ...
    parameters: {
      ...
      method: {
        type: 'string',
        description:
          'Method-level read (classes only): return just this METHOD … ENDMETHOD. block instead of the whole ' +
      ...
      context: {
        type: 'boolean',
        description:
          'Append a DEPENDENCY-CONTRACT PROLOGUE (default false): the public-section contracts of the classes/' +
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          contextPrologue: {
            type: 'string',
            description:
              'Dependency-contract prologue (context: true): ABAP-comment block listing resolved contracts, ' +
      ...
      presentationMeta: (_args, value) =>
        ...
        !value.method && value.startLine <= 1 && value.endLine >= value.totalLines
          ? (readPresentationMeta(value) as unknown as JsonValue)
          : null,
    },
    ...
    execute: async (args, exec) => {
      ...
      if (STRUCTURED_KINDS.has(category)) {
        throw new Error(
          `adt_read_object: ${ref.name} is a ${category} — its content is structured metadata, not source text. ` +
            `Use adt_read_structure (name: ${ref.name}, kind: ${category}) instead.`,
        );
      }
      if (category === 'DEVC') {
        throw new Error(
          `adt_read_object: ${ref.name} is a package — list its objects with adt_package_content (packageName: ${ref.name}).`,
        );
      }
      ...
      if (methodName) {
        ...
        if (blocks.length === 0) {
          ...
          throw new Error(
            `adt_read_object: method "${methodName}" not found in ${ref.name} (${totalLines} lines)` +
        ...
        if (blocks.length > 1) {
          ...
          throw new Error(
            `adt_read_object: method "${methodName}" is defined ${blocks.length} times (local classes?) — ${occurrences}. ` +
      ...
      const endLine = Math.min(
        Math.max(Number.isFinite(requestedEnd) ? requestedEnd : totalLines, startLine),
      ...
      let contextPrologue: string | undefined;
