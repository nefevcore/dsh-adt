...
export function lifecycleTools(deps: ToolDeps) {
  ...
  /** Shape shared by the activation output schema and its renderer. */
  interface ActivationOutputValue {
    success: boolean;
    items: Array<{
      name: string;
      type: string;
      status: string;
      message?: string;
      errors: Array<{ text: string; line?: number; code?: string }>;
    }>;
    hints?: string[];
  }
  ...
  const activationOutput = {
    ...
    render: (_args: unknown, value: ActivationOutputValue) =>
      text(
        [
  ...
  const activate = defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (!checkOnly && result.success && mains.length > 0) {
        hints.push(
          `${mains.map((r) => r.name).join(', ')}: activation of a main program / function group does NOT cascade to its includes (TOP/SCR/...) on some backends — pass the main object AND its includes together in \`objects\` (one call), and verify leftovers with adt_version_diff (saved version vs active)`,
        );
      }
      if (!result.success) {
        hints.push(
          'activation failed — note that adt_check PASSING does not guarantee activation succeeds: the activation ' +
  ...
  const check = defineTool({
    ...
    execute: async (args, exec) => {
      ...
      // round-trips.
      const messages: Array<{ objectName: string; severity: string; text: string; line?: number; code?: string }> = [];
