    ...
    defineTool({
      ...
      parameters: {
        ...
        password: {
          type: 'string',
          description:
            'Password for `username`. Stored in the DSH credential store (~/.dsh/.credentials.yaml) and ' +
        ...
        if (password !== undefined) {
          ...
          if (!passwordInFile && service?.set !== undefined) {
            try {
              ...
              notes.push(
                `password stored in the DSH credential store (reference ${passwordRef}; backing file ~/.dsh/.credentials.yaml) — destinations.yaml keeps only the reference`,
              );
            } catch (error) {
