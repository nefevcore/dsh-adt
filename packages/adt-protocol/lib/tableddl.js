...
export function mapTableFieldType(field) {
    ...
    if (/^CHAR\d+$/.test(t))
        return `abap.char(${t.slice(4)})`;
    if (/^NUMC\d+$/.test(t))
        return `abap.numc(${t.slice(4)})`;
    // Anything else must be a data element reference.
    if (!/^[A-Za-z/][A-Za-z0-9_/]*$/.test(field.type)) {
        throw new Error(`table field ${field.name}: '${field.type}' is neither a builtin type code nor a valid data element name`);
    }
    return field.type.toLowerCase();
}
/** Escape single quotes for DDL string literals ('' doubling). */
function ddlQuote(text) {
 ...
 */
export function generateTableDdl(request) {
...
//# sourceMappingURL=tableddl.js.map