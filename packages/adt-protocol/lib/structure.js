...
export function structureMediaType(kind) {
    switch (kind) {
        case 'MSAG':
...
function prefixedAttr(node, name) {
...
export function parseStructure(xml, kind) {
...
function ttypProperties(root) {
    ...
    if (keyEl) {
        ...
        if (definition !== undefined)
            props.keyDefinition = definition;
        if (kind !== undefined)
            props.keyKind = kind;
    }
    return props;
...
function patchElementText(xml, parentLocal, localName, value) {
...
function replaceElementBlock(xml, localName, block) {
    const range = elementRange(xml, localName);
    if (range)
        return xml.slice(0, range.start) + block + xml.slice(range.end);
...
function insertBeforeRootClose(xml, fragment) {
    const lastClose = xml.lastIndexOf('</');
    if (lastClose < 0)
        return xml + fragment;
 ...
 */
export function patchStructureXml(xml, kind, changes) {
    let out = xml;
    ...
    if (kind === 'MSAG' && changes.messages) {
        ...
        const current = parseStructure(xml, 'MSAG').messages;
        const kept = new Set(changes.messages.map((m) => m.number));
        const lines = [];
        for (const message of changes.messages) {
            lines.push(`<mc:messages mc:msgno="${escape(message.number)}" mc:msgtext="${escape(message.text)}"` +
    ...
    if (kind === 'DOMA') {
        for (const [key, raw] of Object.entries(changes.properties ?? {})) {
            ...
            if (key === 'valueTable') {
                ...
                out = out.replace(/(<(?:[\w.-]+:)?valueTableRef\b[^>]*?)(?:[\w.-]+:)?name\s*=\s*"[^"]*"/, `$1adtcore:name="${escape(value)}"`);
                continue;
            }
            const parent = key === 'conversionExit' || key === 'signExists' || key === 'lowercase' || key === 'outputStyle'
                ? 'outputInformation'
                : 'typeInformation';
        ...
        if (changes.fixedValues) {
            const items = changes.fixedValues
                ...
                `\n      </doma:fixValue>`)
                .join('\n');
            out = replaceElementBlock(out, 'fixValues', changes.fixedValues.length > 0 ? `<doma:fixValues>\n${items}\n    </doma:fixValues>` : '<doma:fixValues/>');
        }
        return out;
    }
    if (kind === 'DTEL') {
