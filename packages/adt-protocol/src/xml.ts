...
class XmlParser {
  ...
  private skipMisc(): void {
    while (this.pos < this.input.length) {
      if (/\s/.test(this.input[this.pos]!)) {
        ...
        if (this.pos > this.input.length) throw new Error('XML: unterminated DOCTYPE');
      } else {
        break;
      }
    }
  ...
  private parseElementInner(): XmlNode {
    const rawName = this.readName();
    if (!rawName) throw new Error(`XML: empty tag name at ${this.pos}`);
    ...
    for (;;) {
      this.skipWs();
      if (this.pos >= this.input.length) throw new Error('XML: unterminated tag');
      const c = this.input[this.pos]!;
      if (c === '>') {
        this.pos++;
        break;
      ...
      if (c === '/') {
        if (this.input[this.pos + 1] !== '>') throw new Error('XML: malformed self-closing tag');
        this.pos += 2;
        return node;
      }
      const attrName = this.readName();
      if (!attrName) throw new Error(`XML: malformed attribute at ${this.pos}`);
      this.skipWs();
      if (this.input[this.pos] !== '=') throw new Error(`XML: expected '=' after attribute ${attrName}`);
      this.pos++;
      this.skipWs();
      const quote = this.input[this.pos];
      if (quote !== '"' && quote !== "'") throw new Error('XML: expected quoted attribute value');
      this.pos++;
    ...
    for (;;) {
      if (this.pos >= this.input.length) throw new Error(`XML: unterminated element <${name}>`);
      if (this.input[this.pos] === '<') {
        if (this.input.startsWith('</', this.pos)) {
          ...
          this.skipWs();
          if (this.input[this.pos] !== '>') throw new Error('XML: malformed closing tag');
          this.pos++;
          if (closeName !== name) {
      ...
      if (next < 0) throw new Error('XML: unterminated element');
  ...
  private readUntil(stop: string): string {
    ...
    if (end < 0) throw new Error(`XML: unterminated quoted value (missing '${stop}')`);
    this.pos = end + 1;
  ...
  private skipUntil(marker: string): void {
    ...
    if (end < 0) throw new Error(`XML: unterminated ${marker}`);
