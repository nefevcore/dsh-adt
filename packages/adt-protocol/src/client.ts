...
function getInsecureTlsDispatcher(): unknown {
  ...
  if (insecureTlsDispatcher !== undefined) return insecureTlsDispatcher;
...
function parseAtcResultList(xml: string): AdtAtcRunSummary[] {
  ...
  for (const el of [...children(root, 'result'), ...children(root, 'entry')]) push(el);
