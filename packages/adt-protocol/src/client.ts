...
import { ENDPOINTS, MEDIA, toQuery, type AdtQueryParams } from './endpoints.js';
...
export class AdtClient {
  ...
  async search(
    query: string,
    options: { maxResults?: number; operation?: string; objectType?: string } = {},
  ...
  async listTransports(options: { allUsers?: boolean; category?: 'K' | 'C' | 'T' } = {}): Promise<AdtTransport[]> {
    const params = this.baseQuery({
  ...
  async packageContent(packageName: string): Promise<AdtObjectRef[]> {
    const params = this.baseQuery({
  ...
  /** Delete an object. */
  async deleteObject(objectUri: string): Promise<void> {
    const uri = normalizeUri(objectUri);
...
function buildCreateObjectRequest(request: AdtCreateObjectRequest): string {
  ...
  const props = Object.entries(request.properties ?? {})
    ...
    .join('\n');
  const nameAttr =
    request.type.startsWith('CLAS') || request.type.startsWith('INTF')
...
function isAtcRunComplete(xml: string): boolean {
  try {
    ...
    if (state && (state.includes('completed') || state.includes('finished') || state.includes('done'))) return true;
