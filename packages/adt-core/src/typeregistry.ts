...
const ROWS: readonly TypeRegistryRow[] = [
...
const BY_TYPE = new Map<string, TypeRegistryRow>();
const BY_ADT_TYPE = new Map<string, TypeRegistryRow>();
for (const row of ROWS) {
  BY_TYPE.set(row.type, row);
  BY_TYPE.set(row.adtType, row);
  BY_ADT_TYPE.set(row.adtType, row);
  for (const alias of row.aliases ?? []) BY_TYPE.set(alias.toUpperCase(), row);
}

/** All rows in canonical (matrix) order. */
export function typeRegistryRows(): readonly TypeRegistryRow[] {
  return ROWS;
}

/** Canonical type short codes (matrix row order). */
export function typeRegistryTypes(): string[] {
  return ROWS.map((r) => r.type);
}

/** Look up a row by short code / adtType / alias (case-insensitive). */
export function typeRegistryRow(type: string): TypeRegistryRow | undefined {
  return BY_TYPE.get((type ?? '').toUpperCase());
}

/** Rows of a given phase (P1..P4). */
export function typeRegistryRowsForPhase(phase: number): readonly TypeRegistryRow[] {
  return ROWS.filter((r) => r.phase === phase);
}

/** Types whose row declares a create endpoint (the creatable catalog). */
export function typeRegistryCreatableTypes(): string[] {
 ...
 */
export function typeRegistryAcceptedSpellings(): string[] {
  const spellings: string[] = [];
  for (const row of ROWS) {
    spellings.push(row.type, row.adtType);
    for (const alias of row.aliases ?? []) spellings.push(alias.toUpperCase());
  }
  return spellings;
}
...
export function typeRegistryDuplicateSpellings(): string[] {
  const seen = new Map<string, number>();
  for (const row of ROWS) {
    seen.set(row.type, (seen.get(row.type) ?? 0) + 1);
    for (const alias of row.aliases ?? []) seen.set(alias.toUpperCase(), (seen.get(alias.toUpperCase()) ?? 0) + 1);
  }
