 ...
 */
export const CRUD_MATRIX: Record<string, Partial<Record<CrudVerb, CrudCell>>> = {
  CLAS: {
...
export const CRUD_VERBS: readonly CrudVerb[] = ['create', 'read', 'update', 'delete'];
...
export function crudObjectTypes(): string[] {
  return [...ROW_ORDER];
}

/** Types with a create cell — the canonical creatable-type list. */
export function crudCreatableTypes(): string[] {
...
export function crudCell(verb: CrudVerb, type: string): CrudCell | undefined {
  const row = CRUD_MATRIX[type.toUpperCase()];
  return row?.[verb];
}

/** The verbs supported for a type, in canonical order. */
export function crudVerbsFor(type: string): CrudVerb[] {
  const row = CRUD_MATRIX[type.toUpperCase()];
  if (!row) return [];
 ...
 * bare "no handler".
 */
export function crudUnsupportedMessage(verb: CrudVerb, type: string): string {
  const upper = type.toUpperCase();
  const row = CRUD_MATRIX[upper];
  if (!row) {
    return (
      `adt_crud: unknown object type '${type}' (known: ${crudObjectTypes().join(', ')}). ` +
...
export function renderCrudMatrixTable(): string {
