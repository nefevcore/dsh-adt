...
const METHOD_OPENER = (name: string) => new RegExp(`^\\s*METHOD\\s+${name}\\s*\\.`, 'i');
 ...
 */
export function findMethodBlocks(source: string, methodName: string): LineBlock[] {
...
// ---------------------------------------------------------------------------
// Dependency extraction (context prologue)
// ---------------------------------------------------------------------------
 ...
 *  exceptions. */
const ROLE_WEIGHT: Record<DepRole, number> = {
  super: 100,
  interface: 95,
  signature: 60,
  collaborator: 40,
...
interface PatternSpec {
  re: RegExp;
  role: DepRole;
  /** true = the capture group is a function-module name (kind 'function'). */
  function?: boolean;
}
 ...
 */
export function extractDependencyCandidates(source: string, selfName: string): DepCandidate[] {
  const lines = source.split(/\r\n|\n/);
  const definedHere = new Set<string>([selfName.toUpperCase()]);
  ...
  const add = (name: string, role: DepRole, line: number, functionKind: boolean): void => {
    const upper = name.toUpperCase().replace(/[\s,.]+$/, '');
    if (upper.length < 3) return; // noise guard
    if (definedHere.has(upper)) return;
    if (!functionKind && LOCAL_PREFIXES.test(upper)) return; // local naming conventions
    if (functionKind) {
      // keep function modules even when they share a prefix with locals
    } else if (!/^[A-Za-z][A-Za-z0-9_\/]*$/.test(upper)) return;
    let cand = byName.get(upper);
    if (!cand) {
      cand = { name: upper, kind: functionKind ? 'function' : 'oo', roles: [], usageCount: 0, line };
      byName.set(upper, cand);
    }
    if (functionKind) cand.kind = 'function';
    if (!cand.roles.includes(role)) cand.roles.push(role);
    cand.usageCount += 1;
  };
      ...
      for (const m of target.matchAll(p.re)) {
        ...
        if (p.role === 'exception') {
          ...
          for (const part of captured.split(',')) {
            ...
            if (name) add(name, 'exception', i + 1, false);
          }
        } else {
...
export function rankDependencies(candidates: DepCandidate[]): DepCandidate[] {
...
function capLines(text: string): string {
  ...
  if (lines.length <= CONTRACT_MAX_LINES) return text.replace(/\r\n/g, '\n');
 ...
 */
export function extractContract(source: string, name: string, type: 'CLAS' | 'INTF'): string {
  if (type === 'INTF') return capLines(source);
  const lines = source.split(/\r\n|\n/);
  const opener = new RegExp(`^\\s*CLASS\\s+${name.replace(/[^A-Za-z0-9_\/]/g, '')}\\s+DEFINITION`, 'i');
  const start = lines.findIndex((l) => opener.test(l));
  if (start < 0) return capLines(source); // unexpected shape — whole source, capped
  const out: string[] = [];
  for (let i = start; i < lines.length; i++) {
    const line = lines[i] ?? '';
    if (i > start && /^\s*(PROTECTED|PRIVATE)\s+SECTION\b/i.test(line)) break;
    if (i > start && /^\s*ENDCLASS\b/i.test(line)) break;
    out.push(line);
  }
