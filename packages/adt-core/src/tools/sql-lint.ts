...
function splitLiterals(sql: string): Array<{ text: string; literal: boolean }> {
  const segments: Array<{ text: string; literal: boolean }> = [];
  const re = /('([^']|'')*'|`[^`]*`)/g;
  let lastIndex = 0;
  for (let m = re.exec(sql); m !== null; m = re.exec(sql)) {
    if (m.index > lastIndex) segments.push({ text: sql.slice(lastIndex, m.index), literal: false });
    segments.push({ text: m[0], literal: true });
    lastIndex = m.index + m[0].length;
  }
  if (lastIndex < sql.length) segments.push({ text: sql.slice(lastIndex), literal: false });
  return segments;
}
...
function countLikes(sql: string): number {
...
function wrapLine(line: string, max: number): string[] {
  ...
  while (rest.length > max) {
    ...
    if (cut <= 0) cut = max; // no whitespace in budget: hard break (rare)
 ...
 */
export function lintFreestyleSql(input: string): SqlLintResult {
  let sql = input.replace(/\r\n/g, '\n').trim();
  const rewrites: SqlLintFinding[] = [];
  ...
  if (!/^\s*select[\s(]/i.test(sql)) {
    throw new SqlLintError(
      'select-only',
      '`sql` accepts a SELECT statement only (the data-preview API is read-only). ' +
  ...
  if (compound) {
    throw new SqlLintError(
      'union',
      `UNION/INTERSECT/EXCEPT compound SELECTs are not accepted by the data-preview parser ` +
  ...
  if (hasOr && likes > 0) {
    throw new SqlLintError(
      'or-like',
      'OR combined with LIKE is rejected by the data-preview parser (live-verified 400). ' +
  ...
  if (likes > 1) {
    throw new SqlLintError(
      'multi-like',
      'the data-preview parser accepts at most ONE LIKE per statement (live-verified 400). ' +
  ...
  if (/"/.test(code)) {
    throw new SqlLintError(
      'double-quote',
      'double quotes are not valid in this dialect (Open SQL uses single quotes for literals ' +
  ...
  if (interiorSemicolon) {
    throw new SqlLintError(
      'multi-statement',
      '`sql` must be ONE SELECT statement — a statement separator (;) inside the text is not ' +
  ...
  if (obMatch) {
    ...
    const fixed = tail.replace(/\b(desc|asc)(?=\s*(?:,|$))/gi, (token) =>
      token.toUpperCase() === 'DESC' ? 'DESCENDING' : 'ASCENDING',
    );
    if (fixed !== tail) {
  ...
  //    left verbatim.
  const padded = mapCode(sql, (c) =>
    c.replace(/\b(count|sum|min|max|avg)\s*\(\s*([^()]*?)\s*\)/gi, (_all, fn: string, inner: string) => `${fn.toUpperCase()}( ${inner} )`),
  );
  if (padded !== sql) {
    sql = padded;
    rewrites.push({
