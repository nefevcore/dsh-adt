...
export function atcRunTools(deps: ToolDeps) {
  ...
  return [
    defineTool({
      name: 'adt_atc_runs',
      description:
        'Inspect ATC (ABAP Test Cockpit) runs STORED on the system. WITHOUT `displayId`: list existing runs — each shows ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          const counts = (value.counts ?? {}) as Partial<Record<'INFO' | 'WARNING' | 'ERROR' | 'CRITICAL' | 'CATASTROPHIC', number>>;
          const findings = (value.findings ?? []) as AtcFindingOutput[];
          const lines = [
