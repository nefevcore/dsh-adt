...
export function transportTools(deps: ToolDeps) {
  ...
  const listTransports = defineTool({
    ...
    description:
      'List transport requests (CTO, Transport Organizer / SE10 / SE09) of the current user: number, status, category, owner and (optionally) contained objects. ' +
    ...
    parameters: {
      ...
      status: {
        type: 'string',
        description:
          'Filter by release state (default "all"): "modifiable" = open/unreleased requests (alias "D"), ' +
  ...
  const getTransport = defineTool({
    ...
    description:
      'Get one transport request (SE10/SE09) including its contained objects (items). NOTE: version histories (adt_object_versions) ' +
