...
import { sessionCwd, DESTINATION_PARAM, OBJECT_REF_PARAMS, VERSION_FEED_META_PROPERTIES, destinationOf, isAdtServiceUnavailable, optStr, resolveToolObject, text, type ToolDeps } from './common.js';
...
export function transportTools(deps: ToolDeps) {
  ...
  const transports = defineTool({
    name: 'adt_transports',
    description:
      'Transport requests (CTO, Transport Organizer / SE10 / SE09). WITHOUT `number`: the list of the current user\'s ' +
    ...
    parameters: {
      ...
      status: {
        type: 'string',
        description:
          'List: filter by release state (default "all"): "modifiable" = open/unreleased requests (alias "D"), ' +
