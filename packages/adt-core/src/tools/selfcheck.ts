...
/** Raw outcome before oracle cross-checks. */
type Raw = { kind: 'ok'; detail: string } | { kind: 'okEmpty'; detail: string } | { kind: 'error'; error: unknown; detail: string };

function classify(raw: Raw): { verdict: SweepVerdict; detail: string } {
  if (raw.kind === 'ok') return { verdict: 'answered', detail: raw.detail };
  if (raw.kind === 'okEmpty') return { verdict: 'empty', detail: raw.detail };
  const error = raw.error;
  if (error instanceof AdtPolicyError) return { verdict: 'refused', detail: `policy refusal (${error.rule}): ${raw.detail}` };
  ...
  return { verdict: 'broken', detail: `${(error as Error).message}` };
}

async function attempt(label: string, run: () => Promise<Raw>): Promise<Raw> {
  try {
    return await run();
  } catch (error) {
...
export function selfcheckTools(deps: ToolDeps) {
  const { registry } = deps;

  const selfcheck = defineTool({
    name: 'adt_selfcheck',
    description:
      'Capability sweep of this destination (READ-ONLY — never writes, never executes code): runs every ' +
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        lines.push(
          '',
          `Summary: ${s.answered} answered, ${s.empty} empty, ${s.dead} DEAD, ${s.absent} absent, ${s.refused} refused, ${s.broken} broken, ${s.skipped} skipped`,
        );
    ...
    execute: async (args, exec) => {
      ...
      let probe: { name: string; type: string; uri: string; packageName?: string } | undefined;
      ...
      if (wanted) {
        ...
        if (exact) probe = { name: exact.objectName, type: exact.type ?? '', uri: exact.uri, packageName: exact.packageName };
        else rows.push({ capability: 'probe selection', tool: 'adt_search', verdict: 'skipped', detail: `no exact match for ${wanted} — pass another name` });
      } else {
        ...
        if (hit) probe = { name: hit.objectName, type: hit.type ?? '', uri: hit.uri, packageName: hit.packageName };
      }
      ...
      const searchCap = await attempt('search', async () => {
        ...
        return hits.length > 0
          ? ({ kind: 'ok', detail: `${hits.length} hit(s) for "${probe?.name ?? 'Z*'}"` } as const)
          : ({ kind: 'okEmpty', detail: 'no hits' } as const);
      });
      ...
      const readCap = await attempt('read', async () => {
        ...
        return parsed.source.trim().length > 0
          ? ({ kind: 'ok', detail: `${parsed.source.split('\n').length} lines` } as const)
          : ({ kind: 'okEmpty', detail: 'empty source' } as const);
      });
      ...
      const packageCap = await attempt('package_content', async () => {
        ...
        return contains
          ? ({ kind: 'ok', detail: `${probe!.packageName} lists ${refs.length} object(s) including ${probe!.name}` } as const)
          : ({ kind: 'okEmpty', detail: `${probe!.packageName} (${refs.length} objects) does not list ${probe!.name}` } as const);
      });
      ...
      const whereUsedCap = await attempt('where_used', async () => {
        ...
        return result.totalReferences > 0
          ? ({ kind: 'ok', detail: `${result.totalReferences} referencing object(s)` } as const)
          : ({ kind: 'okEmpty', detail: 'no references (truthful for a fresh object)' } as const);
      });
      ...
      const versionsCap = await attempt('object_versions', async () => {
        ...
        return versions.length > 0
          ? ({ kind: 'ok', detail: `${versions.length} version(s)` } as const)
          : ({ kind: 'okEmpty', detail: 'no version feed' } as const);
      });
      ...
      const activeCap = await attempt('active_version', async () => {
        ...
        return parsed.source.trim().length > 0
          ? ({ kind: 'ok', detail: 'active version readable' } as const)
          : ({ kind: 'okEmpty', detail: 'empty active version' } as const);
      });

      const dumpsCap = await attempt('dumps_list', async () => {
        const dumps = await client.listDumps({ signal: exec.signal });
        return dumps.length > 0
          ? ({ kind: 'ok', detail: `${dumps.length} dump(s)` } as const)
          : ({ kind: 'okEmpty', detail: 'no dumps (truthful on a healthy system)' } as const);
      });
      ...
      const transportsCap = await attempt('transports_list', async () => {
        ...
        return transports.length > 0
          ? ({ kind: 'ok', detail: `${transports.length} transport(s)` } as const)
          : ({ kind: 'okEmpty', detail: 'no open transports (truthful)' } as const);
      });

      const pingCap = await attempt('ping', async () => {
        const status = await client.ping({ signal: exec.signal });
        return status.ok ? ({ kind: 'ok', detail: 'reachable' } as const) : ({ kind: 'okEmpty', detail: 'endpoint answered but not ok' } as const);
      });

      const systemCap = await attempt('system_info', async () => {
        const info = await client.systemInfo({ signal: exec.signal });
        return { kind: 'ok' as const, detail: `${info.systemId || '?'} ${info.release || ''}`.trim() || 'system info' };
      });
      ...
      const batchCap = await attempt('batch_get', async () => {
        ...
        if (typeof body !== 'string' || body.length === 0) return { kind: 'okEmpty' as const, detail: '$batch GET returned no body' };
        if (body !== readBack.source) return { kind: 'error', error: new Error('$batch body differs from a direct read'), detail: 'batch_get' };
        return { kind: 'ok' as const, detail: '$batch GET matches a direct read' };
      });

      // Second-level probe: dump detail only when the feed has rows.
      let dumpDetailCap: Raw = { kind: 'okEmpty', detail: 'not attempted: dump feed is empty' };
      if (dumpsCap.kind === 'error') dumpDetailCap = { kind: 'okEmpty', detail: 'not attempted: dump feed unavailable' };
      if (dumpsCap.kind === 'ok') {
        dumpDetailCap = await attempt('get_dump', async () => {
          ...
          return (detail.raw ?? '').length > 0
            ? ({ kind: 'ok', detail: `dump ${first.id.slice(0, 16)}… readable` } as const)
            : ({ kind: 'okEmpty', detail: 'dump detail empty' } as const);
        });
      }
      ...
      const push = (capability: string, tool: string, raw: Raw, oracleDead?: boolean): void => {
        if (oracleDead) {
          rows.push({
            capability,
            tool,
      ...
      const pushObj = (capability: string, tool: string, raw: Raw, oracleDead?: boolean): void => {
        if (!probe) {
      ...
      return {
        ...
        note:
          'Read-only sweep: capabilities that write or execute code (activate/edit/execute/unit tests/ATC/gate/' +
