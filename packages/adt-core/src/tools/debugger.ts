...
export function debuggerTools(deps: ToolDeps) {
  ...
  return [
    defineTool({
      name: 'adt_debug',
      description:
        'Debug ABAP code live over the standard ADT debugger (SE37-style new debugger, /sap/bc/adt/debugger) — one ' +
      ...
      parameters: {
        action: {
          ...
          required: true,
          description:
            'listen = wait for a breakpoint hit (long-poll); status = session + listener state; detach = end the session; ' +
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          if (value.backendListeners) lines.push(`backend listeners: ${value.backendListeners}`);
          if (value.breakpoints?.length) {
            for (const b of value.breakpoints as AdtDebugBreakpoint[]) {
              lines.push(`- breakpoint id ${b.id} → ${b.uri}${b.line ? ` line ${b.line}` : ''}`);
            }
            lines.push('next: trigger the code path, then adt_debug {action:"listen"}');
          }
          if (value.deleted !== undefined) lines.push(value.deleted ? 'breakpoint deleted' : (value.note ?? ''));
      ...
      execute: async (args, exec) => {
        ...
        if (action === 'setVariable') entry.policy.assertDebugVariablesAllowed('adt_debug');
        ...
        if (action === 'setBreakpoint') {
          ...
          if (typeof line !== 'number' || line < 1) throw new Error('adt_debug: `line` (1-based) is required for setBreakpoint');
          let uri = optStr(args.objectUri);
          if (!uri) {
            const name = optStr(args.name);
            if (!name) throw new Error('adt_debug: provide objectUri or name(+type) for setBreakpoint');
        ...
        if (action === 'deleteBreakpoint') {
          ...
          if (!id) throw new Error('adt_debug: `id` (from setBreakpoint) is required for deleteBreakpoint');
          const record = manager.sessionFor(entry, user);
        ...
        if (action === 'variables') {
          const names = Array.isArray(args.variables) ? args.variables.map(String).filter(Boolean) : [];
          if (names.length === 0) throw new Error('adt_debug: `variables` (non-empty name list) is required for the variables action');
        ...
        if (action === 'setVariable') {
          ...
          if (!name || value === undefined) throw new Error('adt_debug: `name` and `value` are required for setVariable');
