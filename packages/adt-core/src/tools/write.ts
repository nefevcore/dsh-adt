...
const WRITE_STATUS_SCHEMA = {
  ...
  persisted: {
    type: 'boolean',
    description:
      'Whether a post-write read-back still matched what was written. false = a concurrent editor overwrote ' +
...
/** The value shape WRITE_STATUS_SCHEMA describes (renderer-tolerant: schema
 * typing marks non-required fields optional). */
interface WriteStatus {
  ...
  warning?: string;
  transport?: string;
  transportSource?: 'user' | 'auto';
  activation?: { success?: boolean; message?: string };
}

/** Render suffix shared by the write-family tools (lock/activation/transport/persistence). */
function writeStatusSuffix(value: WriteStatus): string {
  return (
 ...
 */
async function lockedWritePipeline(
  entry: RegistryDestination,
  ledger: LockLedger,
  params: {
    toolName: string;
    ref: AdtObjectRef;
    /** User-specified transport (already policy-checked by the caller). */
    transport?: string;
    /** Keep the lock on success (adt_write_object `unlock:false`)? */
    unlockOnSuccess: boolean;
    activate: boolean;
    signal?: AbortSignal;
    /** Compute the source to write; may read/verify server state first. */
    produce: () => Promise<string>;
  },
): Promise<WriteStatus & { written?: string }> {
  ...
  try {
    ...
    if (!succeeded || params.unlockOnSuccess) {
      const released = await entry.client
        ...
        .then(() => true)
        .catch(() => false);
      if (released) ledger.deregister(entry.config.name, params.ref.uri);
      unlocked = released;
    }
  }
  let activated = false;
  let activation: WriteStatus['activation'];
  if (params.activate && succeeded) {
    try {
      ...
      activated = false;
      activation = { success: false, message: (error as Error).message };
    }
  }
  return {
    unlocked,
...
export function writeTools(deps: ToolDeps, ctx: ToolHost) {
  ...
  const editSource = defineTool({
    ...
    parameters: {
      ...
      method: {
        type: 'string',
        description:
          'Mode 3 (method surgery, classes): replace exactly ONE METHOD … ENDMETHOD. block. Give the method name ' +
    ...
    execute: async (args, exec) => {
      ...
      if (methodName) {
        if (oldText !== undefined || startText || hasStartLine || args.end !== undefined || args.endLine !== undefined) {
          throw new Error(
            'adt_edit_object: `method` cannot be combined with `oldText`/`start`/`end`/`startLine`/`endLine` — ' +
        ...
        throw new Error(
          'adt_edit_object: provide `oldText`+`newText` (quote-and-replace, preferred), `method`+`newText` ' +
      ...
      const replacement =
        oldText !== undefined || methodName !== undefined ? String(args.newText ?? '') : await resolveSourceInput(ctx, args);
      ...
      const outcome = await lockedWritePipeline(entry, ledger, {
        ...
        produce: async () => {
          ...
          const replacementText = oldText !== undefined || methodName !== undefined ? String(args.newText ?? '') : (replacement as string);
          if (methodName) {
            ...
            if (blocks.length === 0) {
              throw new Error(
                `adt_edit_object: method "${methodName}" not found in the current source of ${ref.name} — ` +
            ...
            if (blocks.length > 1) {
              ...
              throw new Error(
                `adt_edit_object: method "${methodName}" is defined ${blocks.length} times (local classes?) — ${occurrences}. ` +
  ...
  const pushObject = defineTool({
    ...
    description:
      'Upload a locally edited object snapshot to the server with conflict verification — the push half of ' +
    ...
    parameters: {
      ...
      path: {
        type: 'string',
        description:
          'Local file to upload. Default: the tracked snapshot from adt_read_object (see its `localCopy` ' +
      ...
      transport: {
        type: 'string',
        description:
          'Transport request number the change is recorded into. Omitted → backend decides on lock ' +
    ...
    output: {
      ...
      render: (_args, value) =>
        text(
          `${value.name}: pushed ${value.localCopy} → server` +
    ...
    execute: async (args, exec) => {
      ...
      if (customPath) {
        ...
        if (!snapshot) {
          throw new Error(
            `adt_push_object: no tracked snapshot for ${ref.name} — run adt_read_object first (it creates the ` +
        ...
        if (!snapshot) {
          throw new Error(
            `adt_push_object: no local snapshot for ${ref.name} — run adt_read_object first (it creates one; ` +
