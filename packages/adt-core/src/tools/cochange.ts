...
import { sessionCwd, showingOfTotal, DESTINATION_PARAM, destinationOf, optStr, resolveToolObject, text, type ToolDeps } from './common.js';

/** One co-occurring object with the transports it shared with the inputs. */
interface CoChangeRow {
  name: string;
  type: string;
  /** How many of the analyzed transports also carried this object. */
  sharedTransports: number;
  /** The shared transport numbers (bounded; the count is the truth). */
  transports: string[];
  description?: string;
}

export function cochangeTools(deps: ToolDeps) {
  const { registry } = deps;
  ...
  return [
    defineTool({
      name: 'adt_cochange',
      description:
        'Find what usually changes together with the given ABAP objects (transport co-occurrence, a slice of ' +
      ...
      parameters: {
        objects: {
          ...
          required: true,
          description:
            'Objects to analyze. Each: {name, type?} — resolve like everywhere else (1..10 per call).',
          items: {
            type: 'object',
      ...
      output: {
        ...
        render: (_args, value) => {
          const lines = [
            ...
            ...value.coChanges.map(
              (row) => `- ${row.name}${row.type ? ` (${row.type})` : ''}: ${row.sharedTransports} shared transport(s) [${row.transports.join(', ')}]`,
            ),
      ...
      execute: async (args, exec) => {
        ...
        const rawObjects = Array.isArray(args.objects) ? (args.objects as Array<Record<string, unknown>>) : [];
        ...
        for (const raw of rawObjects) {
          ...
          try {
            ...
            for (const version of versions) {
              if (version.transportRequest) transportSet.add(version.transportRequest.toUpperCase());
            }
          } catch (error) {
            ...
            notes.push(`no version feed for ${ref.name} (${(error as Error).message})`);
          }
        }

        // 2. Expand each transport into its item list (parent requests).
        const analyzed: string[] = [];
        const counts = new Map<string, { type: string; transports: Set<string>; description?: string }>();
        for (const number of transportSet) {
          if (analyzed.length >= maxTransports) {
            notes.push(
              `transport budget reached — analyzed ${analyzed.length} of ${transportSet.size} request(s); raise maxTransports for deeper history`,
            );
            break;
          }
          try {
            ...
            for (const item of transport.items ?? []) {
              ...
              const existing = counts.get(key) ?? { type: item.type, transports: new Set<string>(), description: item.description };
            ...
            if (exec.signal?.aborted) throw error;
            notes.push(`transport ${number} not readable (${(error as Error).message})`);
          }
        }

        // 3. Rank by shared-transport count (then name for determinism).
        const rows: CoChangeRow[] = [...counts.entries()]
          .map(([key, value]) => ({
            name: key.split('|')[0]!,
            type: value.type,
