...
type TokenType = 'ident' | 'keyword' | 'number' | 'string' | 'op' | 'punct' | 'star';

interface Token {
  type: TokenType;
  /** Upper-cased for ident/keyword, verbatim otherwise. */
  text: string;
  /** Original casing (identifiers keep theirs for error messages). */
  raw: string;
  pos: number;
}
...
function tokenize(sql: string): Token[] {
  ...
  while (i < n) {
    ...
    if (ch === "'") {
      ...
      if (j >= n) throw new SqlCompilerError('unterminated-string', `unterminated string literal at position ${i}`);
...
interface AggregateRef {
  ...
  distinct: boolean;
  arg: ColumnRef; // column or '*'
}

type SelectItem = { expr: ColumnRef | AggregateRef; alias?: string };
...
type Cond =
  ...
  | { kind: 'or'; parts: Cond[] }
  | { kind: 'not'; inner: Cond }
  | { kind: 'cmp'; left: ColumnRef | Literal | AggregateRef; op: '=' | '!=' | '<' | '<=' | '>' | '>=' | 'LIKE'; right: ColumnRef | Literal }
  | { kind: 'in'; left: ColumnRef; not: boolean; list: Literal[] }
  | { kind: 'in-sub'; left: ColumnRef; not: boolean; sub: SelectAst }
  | { kind: 'isnull'; left: ColumnRef; not: boolean };

type Literal = { kind: 'literal'; value: string; numeric: boolean };
...
class Parser {
  ...
  private next(): Token {
    const t = this.peek();
    if (!t) throw new SqlCompilerError('unexpected-end', 'statement ended unexpectedly');
    this.pos += 1;
    return t;
  }
  ...
  parseSelect(subselect = false): SelectAst {
    ...
    if (this.acceptKeyword('WHERE')) where = this.parseCond();
    ...
    if (this.acceptKeyword('HAVING')) having = this.parseCond();
    ...
    if (this.acceptKeyword('ORDER')) {
      ...
      for (;;) {
        ...
        if (this.acceptKeyword('ASCENDING') || this.acceptKeyword('ASC')) dir = 'ASC';
        else if (this.acceptKeyword('DESCENDING') || this.acceptKeyword('DESC')) dir = 'DESC';
    ...
    if (this.acceptKeyword('LIMIT')) {
      this.next(); // the number
      if (this.acceptKeyword('OFFSET')) this.next(); // its number
    }
  ...
  private parseJoinType(): JoinType | undefined {
    const t = this.peek();
    if (!t || t.type !== 'keyword') return undefined;
    if (t.text === 'JOIN') {
      this.pos += 1;
      return 'inner';
    }
    ...
    if (t.text === 'RIGHT' || t.text === 'FULL') {
      throw new SqlCompilerError(
        'join-type',
        `${t.text} JOIN is not supported by the client-side compiler (INNER and LEFT only) — ` +
  ...
  private parseTableRef(): TableRef {
    ...
    const ref: TableRef = { name: t.text };
    if (this.acceptKeyword('AS')) {
      const a = this.next();
      if (a.type !== 'ident') throw new SqlCompilerError('table-alias', `expected an alias after AS but found '${a.raw}'`);
      ref.alias = a.text;
    } else {
      const a = this.peek();
  ...
  private parseSelectExpr(): ColumnRef | AggregateRef {
    const t = this.peek();
    if (!t) throw new SqlCompilerError('unexpected-end', 'expected an expression');
    if (t.type === 'keyword' && (t.text === 'COUNT' || t.text === 'SUM' || t.text === 'MIN' || t.text === 'MAX' || t.text === 'AVG')) {
      this.pos += 1;
      if (!this.acceptPunct('(')) throw new SqlCompilerError('aggregate-syntax', `expected ( after ${t.text}`);
      const distinct = this.acceptKeyword('DISTINCT');
      let arg: ColumnRef;
      ...
      if (!this.acceptPunct(')')) throw new SqlCompilerError('aggregate-syntax', `expected ) to close ${t.text}(`);
  ...
  private parseColumnRef(): ColumnRef {
    const t = this.next();
    if (t.type === 'star') return { kind: 'column', column: '*' };
    if (t.type === 'ident') {
    ...
    if (t.type === 'number' || t.type === 'string') {
      throw new SqlCompilerError(
        'literal-item',
        `literal '${t.raw}' in the SELECT list is not supported — select real columns (or wrap literals in a WHERE condition)`,
      );
    }
  ...
  private parseOr(): Cond {
    ...
    while (this.acceptKeyword('OR')) parts.push(this.parseAnd());
    return parts.length === 1 ? parts[0]! : { kind: 'or', parts };
  }

  private parseAnd(): Cond {
    const parts = [this.parseUnary()];
    while (this.acceptKeyword('AND')) parts.push(this.parseUnary());
    return parts.length === 1 ? parts[0]! : { kind: 'and', parts };
  }

  private parseUnary(): Cond {
    if (this.acceptKeyword('NOT')) return { kind: 'not', inner: this.parseUnary() };
    if (this.acceptPunct('(')) {
      const inner = this.parseOr();
      if (!this.acceptPunct(')')) throw new SqlCompilerError('paren', 'expected ) to close a condition group');
      return inner;
    }
    return this.parsePredicate();
  }

  private parsePredicate(): Cond {
    ...
    if (left.kind !== 'column') {
      ...
      if (left.kind === 'aggregate') {
        ...
        if (t?.type === 'op') {
          ...
          return { kind: 'cmp', left, op: t.text as 'LIKE', right };
        }
      }
      throw new SqlCompilerError(
        'operand',
        'the left side of a condition must be a column reference (literals may only appear on the right)',
      );
    }
    const t = this.peek();
    if (t?.type === 'op') {
  ...
  private parseInAfterKeyword(left: ColumnRef, not: boolean): Cond {
    if (!this.acceptPunct('(')) throw new SqlCompilerError('in-syntax', 'expected ( after IN');
    if (this.peek()?.type === 'keyword' && this.peek()!.text === 'SELECT') {
      const sub = this.parseSubselect();
      if (!this.acceptPunct(')')) throw new SqlCompilerError('in-syntax', 'expected ) to close IN ( SELECT …');
      return { kind: 'in-sub', left, not, sub };
    }
    const list: Literal[] = [];
    for (;;) {
      const t = this.next();
      if (t.type === 'string') list.push({ kind: 'literal', value: t.text, numeric: false });
      else if (t.type === 'number') list.push({ kind: 'literal', value: t.text, numeric: true });
      else throw new SqlCompilerError('in-list', `IN lists take literals — found '${t.raw}'`);
      if (!this.acceptPunct(',')) break;
    }
    if (!this.acceptPunct(')')) throw new SqlCompilerError('in-syntax', 'expected ) to close the IN list');
    return { kind: 'in', left, not, list };
  }

  /** A parenthesized full SELECT (the body of IN ( SELECT … )). */
  private parseSubselect(): SelectAst {
    return this.parseSelect(true);
  }

  private parseOperand(): ColumnRef | Literal {
    const t = this.next();
    if (t.type === 'string') return { kind: 'literal', value: t.text, numeric: false };
    if (t.type === 'number') return { kind: 'literal', value: t.text, numeric: true };
    if (t.type === 'star') throw new SqlCompilerError('operand', "unexpected '*' in a condition");
...
export function collectTables(ast: SelectAst): string[] {
  const names = new Set<string>();
  const walk = (node: SelectAst): void => {
    names.add(node.from.name);
    for (const j of node.joins) names.add(j.table.name);
    const walkCond = (c: Cond | undefined): void => {
      if (!c) return;
      if (c.kind === 'and' || c.kind === 'or') c.parts.forEach(walkCond);
      else if (c.kind === 'not') walkCond(c.inner);
      else if (c.kind === 'in-sub') walk(c.sub);
    };
    walkCond(node.where);
...
export function needsCompilation(ast: SelectAst): boolean {
  ...
  const hasInSub = (c: Cond | undefined): boolean => {
    if (!c) return false;
    if (c.kind === 'and' || c.kind === 'or') return c.parts.some(hasInSub);
    if (c.kind === 'not') return hasInSub(c.inner);
...
export function parseSelect(sql: string): SelectAst {
 ...
 */
export function stripSingleTableAlias(sql: string): string | null {
  let ast: SelectAst;
  try {
    ast = parseSelect(sql);
  } catch {
  ...
  if (ast.joins.length > 0) return null; // multi-table: alias is load-bearing
  ...
  const rewriteList = (text: string): string =>
    text
      .split(',')
      .map((part) => strip(part))
      .join(', ');
...
/** One joined/working row: alias → (column → value). */
type WorkRow = Map<string, Record<string, string>>;

function literalOf(l: Literal): string {
  return l.value;
}
...
/** How a table was fetched — feeds the fidelity notes. */
...
export interface RunQueryFn {
  (sql: string, opts: { top: number; signal?: AbortSignal }): Promise<{
...
function buildFetch(
  ...
  const conds = pushed.map((c) => condToSqlBare(c)).filter((s): s is string => s !== null);
  if (conds.length > 0) sql += ` WHERE ${conds.join(' AND ')}`;
  return sql;
}

/** Render a pushed condition against the ONE fetched table, bare names. */
function condToSqlBare(c: Cond): string | null {
  switch (c.kind) {
    case 'and':
      return c.parts.map((p) => condToSqlBare(p)).filter(Boolean).length === c.parts.length
        ...
        : null;
    case 'cmp': {
      if (c.left.kind !== 'column') return null;
      if (c.right.kind !== 'literal') return null;
      const right = literalOf(c.right);
      if (c.op === 'LIKE') return `${c.left.column} LIKE '${right.replace(/'/g, "''")}'`;
...
function condToSql(c: Cond, alias: string): string | null {
  switch (c.kind) {
    case 'and':
      return c.parts.map((p) => condToSql(p, alias)).filter(Boolean).length === c.parts.length
        ...
        : null;
    case 'cmp': {
      if (c.left.kind !== 'column' || c.left.alias !== alias) return null;
      if (c.right.kind !== 'literal') return null;
      const right = literalOf(c.right);
      const col = `${alias}~${c.left.column}`;
      if (c.op === 'LIKE') return `${col} LIKE '${right.replace(/'/g, "''")}'`;
      return `${col} ${c.op} ${c.right.numeric ? right : `'${right.replace(/'/g, "''")}'`}`;
    }
    case 'in': {
      if (c.left.alias !== alias) return null;
      const values = c.list.map((l) => (l.numeric ? l.value : `'${l.value.replace(/'/g, "''")}'`)).join(', ');
...
function condColumns(c: Cond, out: Array<{ alias?: string; column: string }>): void {
  if (c.kind === 'and' || c.kind === 'or') {
    c.parts.forEach((p) => condColumns(p, out));
  } else if (c.kind === 'not') {
    condColumns(c.inner, out);
  } else if (c.kind === 'cmp') {
    if (c.left.kind === 'column') out.push({ alias: c.left.alias, column: c.left.column });
    if (c.right.kind === 'column') out.push({ alias: c.right.alias, column: c.right.column });
...
export async function compileAndRun(
...
): Promise<CompiledSqlResult> {
  const ast = parseSelect(sql);
  const notes: string[] = [];
  const executedSqls: string[] = [];
  const fetches: TableFetch[] = [];

  // ---- 1. resolve IN ( SELECT … ) subqueries up front ----------------------
  const subResults = new Map<Cond, string[]>();
  const resolveSubqueries = async (c: Cond | undefined): Promise<void> => {
    if (!c) return;
    if (c.kind === 'and' || c.kind === 'or') {
      for (const p of c.parts) await resolveSubqueries(p);
      return;
    }
    if (c.kind === 'not') return resolveSubqueries(c.inner);
    if (c.kind !== 'in-sub') return;
    const subAst = c.sub;
  ...
  const aliasOf = (t: TableRef): string => t.alias ?? t.name;
  const tables = [ast.from, ...ast.joins.map((j) => j.table)];
  const aliasSeen = new Set<string>();
  for (const t of tables) {
    const a = aliasOf(t);
    if (aliasSeen.has(a)) throw new SqlCompilerError('duplicate-alias', `table alias '${a}' used twice — give each table a unique alias`);
    aliasSeen.add(a);
  }
  ...
  const widen = new Set<string>(); // aliases whose SELECT includes * or t.*
  const note = (a: string, col: string): void => {
    if (col === '*') {
      widen.add(a);
      return;
    }
    const set = referenced.get(a) ?? new Set<string>();
    set.add(col);
    referenced.set(a, set);
  };
  for (const item of ast.items) {
    const e = item.expr;
    if (e.kind === 'column') note(e.alias ?? aliasOf(ast.from), e.column === '*' ? '*' : e.column);
    else if (e.arg.kind === 'column' && e.arg.column !== '*') note(e.arg.alias ?? aliasOf(ast.from), e.arg.column);
    else if (e.arg.kind === 'column') note(e.arg.alias ?? aliasOf(ast.from), '*');
  }
  for (const o of ast.orderBy) {
    const e = o.expr;
    if (e.kind === 'column') note(e.alias ?? aliasOf(ast.from), e.column);
    else if (e.arg.kind === 'column' && e.arg.column !== '*') note(e.arg.alias ?? aliasOf(ast.from), e.arg.column);
  }
  for (const g of ast.groupBy) note(g.alias ?? aliasOf(ast.from), g.column);
  const allConds: Cond[] = [ast.where, ast.having, ...ast.joins.map((j) => j.on)].filter((c): c is Cond => c !== undefined);
  for (const c of allConds) {
    const cols: Array<{ alias?: string; column: string }> = [];
    condColumns(c, cols);
  ...
  const localWhere: Cond[] = [];
  const pushedByAlias = new Map<string, Cond[]>();
  const pushable = (c: Cond): string | null => {
    // Returns the single alias a condition is bound to, or null (mixed/complex).
    const cols: Array<{ alias?: string; column: string }> = [];
    condColumns(c, cols);
    if (cols.some((x) => x.column === '*')) return null;
    const aliases = new Set(cols.map((x) => x.alias));
    if (aliases.size !== 1) return null;
    const only = [...aliases][0];
    if (c.kind === 'cmp' && c.right.kind !== 'literal') return null; // col-to-col stays local
    if (c.kind === 'in-sub') return null; // membership computed locally
    return only ?? null;
  };
  const splitWhere = (c: Cond): void => {
    if (c.kind === 'and') {
  ...
  if (ast.where) splitWhere(ast.where);
  ...
  const tableData = new Map<string, { columns: Array<{ name: string; type: string }>; rows: Array<Record<string, string>> }>();
  for (const t of tables) {
    const a = aliasOf(t);
    const wantAll = widen.has(a);
    const cols = wantAll ? new Set<string>() : referenced.get(a) ?? new Set<string>();
  ...
  if (truncatedFetch.length > 0) {
    notes.push(
      `row-cap approximation: ${truncatedFetch.map((f) => `${f.table} (fetched ${f.fetched}, cap ${f.requestedTop})`).join(', ')} — ` +
  ...
  let rows: WorkRow[] = (tableData.get(aliasList[0]!)?.rows ?? []).map((r) => {
    const m = new Map<string, Record<string, string>>();
    m.set(aliasList[0]!, r);
    return m;
  });
  const baseColumns = new Map<string, Array<{ name: string; type: string }>>();
  for (const a of aliasList) baseColumns.set(a, tableData.get(a)?.columns ?? []);

  const evalCondOnRow = (c: Cond, row: WorkRow, subs: Map<Cond, string[]>): boolean => {
    switch (c.kind) {
      case 'and':
      ...
      case 'cmp': {
        ...
        if (c.left.kind === 'aggregate') return false;
  ...
  for (const join of ast.joins) {
    ...
    const pairs: Array<{ left: ColumnRef; right: ColumnRef }> = [];
    const residuals: Cond[] = [];
    const splitOn = (c: Cond): void => {
      if (c.kind === 'and') {
    ...
    if (pairs.length === 0) {
      throw new SqlCompilerError(
        'join-keys',
        `the ON condition of ${join.table.name} has no equality pair (a~x = b~y) — the local join needs at least one`,
      );
    }
    // Hash the right side by its key columns.
    const rightIndex = new Map<string, Array<Record<string, string>>>();
    for (const rr of rightData.rows) {
      ...
      else rightIndex.set(key, [rr]);
    }
    const joined: WorkRow[] = [];
    for (const row of rows) {
      ...
      if (matches.length > 0) {
        for (const m of matches) {
          ...
          if (residuals.every((c) => evalCondOnRow(c, next, subResults))) joined.push(next);
        }
      } else if (join.type === 'left') {
        // LEFT JOIN: keep the row with NULL (empty) right-side columns.
        const empty: Record<string, string> = {};
        for (const col of rightData.columns) empty[col.name] = '';
        const next = new Map(row);
  ...
  if (localWhere.length > 0) {
    const cond: Cond = localWhere.length === 1 ? localWhere[0]! : { kind: 'and', parts: localWhere };
    rows = rows.filter((row) => evalCondOnRow(cond, row, subResults));
    notes.push(
      'WHERE partially pushed down: ' +
        `${localWhere.length} condition(s) evaluated client-side (cross-table / OR / subquery parts)`,
    );
  }

  // ---- 7. projection / aggregation ------------------------------------------
  const outputName = (item: SelectItem, idx: number): string => {
    if (item.alias) return item.alias;
    if (item.expr.kind === 'column') return item.expr.column === '*' ? `${aliasOf(ast.from)}~*` : item.expr.column;
  ...
  const outColumns: Array<{ name: string; type: string }> = ast.items.map((item, idx) => ({
  ...
  if (!hasAggregate) {
    ...
    if (groupRefs.length === 0 && rows.length === 0) groups.set('', []); // COUNT(*) over empty → 0
    outRows = [];
    for (const [, group] of groups) {
      ...
      if (ast.having) {
        const havingRow: WorkRow = group.length > 0 ? group[0]! : new Map();
        const outputNames = ast.items.map((item, idx) => ({ expr: item.expr, name: outputName(item, idx) }));
        if (!evalHaving(ast.having, rec, havingRow, tables, subResults, outputNames)) continue;
      }
      outRows.push(rec);
    }
  ...
  if (ast.orderBy.length > 0) {
    const keyOf = (r: Record<string, string>, o: OrderItem): string => {
...
function valueOf(ref: ColumnRef, row: WorkRow, tables: TableRef[]): string | undefined {
...
function guessType(
  item: SelectItem,
  baseColumns: Map<string, Array<{ name: string; type: string }>>,
  defaultAlias: string,
): string {
  if (item.expr.kind !== 'column') return 'AGGREGATE';
  if (item.expr.column === '*') return 'STRUCT';
  const cols = baseColumns.get(item.expr.alias ?? defaultAlias) ?? [];
  return cols.find((c) => c.name.toUpperCase() === item.expr.kind && false) ? 'CHAR' : (cols.find((c) => c.name.toUpperCase() === (item.expr as ColumnRef).column.toUpperCase())?.type ?? 'CHAR');
}
...
function aggregateOf(expr: AggregateRef, rows: WorkRow[], tables: TableRef[]): string {
  const values = rows
    .map((row) => (expr.arg.column === '*' ? '1' : valueOf(expr.arg, row, tables) ?? ''))
    .filter((v) => v !== '');
  const uniq = expr.distinct ? [...new Set(values)] : values;
  const fn = expr.fn;
  if (fn === 'COUNT') return String(uniq.length);
...
function orderByOutputName(o: OrderItem): string {
  if (o.expr.kind === 'column') return o.expr.column;
...
function evalHaving(
  ...
  sampleRow: WorkRow,
  tables: TableRef[],
  subs: Map<Cond, string[]>,
  outputNames: Array<{ expr: ColumnRef | AggregateRef; name: string }> = [],
): boolean {
  switch (c.kind) {
    case 'and':
    ...
    case 'cmp': {
      const side = (operand: ColumnRef | Literal | AggregateRef): string | undefined => {
        ...
        if (operand.kind === 'aggregate') {
          ...
          const item = outputNames.find((o) => o.expr.kind === 'aggregate' && o.expr.fn === operand.fn && o.expr.arg.kind === operand.arg.kind && (o.expr.arg.kind !== 'column' || operand.arg.kind !== 'column' || (o.expr.arg as ColumnRef).column === (operand.arg as ColumnRef).column));
          return item ? rec[item.name] : undefined;
        }
        if (operand.alias) return valueOf(operand, sampleRow, tables);
        return rec[operand.column] ?? valueOf(operand, sampleRow, tables);
      };
      const l = side(c.left as ColumnRef);
      const r = side(c.right as ColumnRef);
      if (l === undefined || r === undefined) return false;
      switch (c.op) {
        case '=':
          return compareValues(l, r) === 0;
...
function renderAst(ast: SelectAst): string {
  const parts: string[] = ['SELECT'];
  if (ast.distinct) parts.push('DISTINCT');
  parts.push(
    ast.items
      .map((i) => {
        const e =
  ...
  if (ast.where) {
    ...
    if (rendered) parts.push(`WHERE ${rendered}`);
  }
  if (ast.groupBy.length > 0) parts.push(`GROUP BY ${ast.groupBy.map((g) => (g.alias ? `${g.alias}~${g.column}` : g.column)).join(', ')}`);
  if (ast.orderBy.length > 0) {
...
function renderCond(c: Cond): string | null {
  switch (c.kind) {
    case 'and':
    case 'or':
