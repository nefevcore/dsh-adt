 ...
 */
export const TYPE_MAP: Record<string, { type: string; uriPrefix: string; label: string }> = {
  ...
  'DDLS/DF': { type: 'DDLS/DF', uriPrefix: '/sap/bc/adt/ddic/ddl/sources/', label: 'CDS Data Definition' },
  ...
  DCLS: { type: 'DCLS/DL', uriPrefix: '/sap/bc/adt/acm/dcl/sources/', label: 'CDS Access Control' },
  'DCLS/DL': { type: 'DCLS/DL', uriPrefix: '/sap/bc/adt/acm/dcl/sources/', label: 'CDS Access Control' },
  BDEF: { type: 'BDEF/BDO', uriPrefix: '/sap/bc/adt/bo/behaviordefinitions/', label: 'Behavior Definition' },
  'BDEF/BDO': { type: 'BDEF/BDO', uriPrefix: '/sap/bc/adt/bo/behaviordefinitions/', label: 'Behavior Definition' },
  SRVD: { type: 'SRVD/SRV', uriPrefix: '/sap/bc/adt/ddic/srvd/sources/', label: 'Service Definition' },
...
export function normalizeType(type: string | undefined): { type: string; uriPrefix: string; label: string } {
...
export function refFromName(name: string, type?: string): AdtObjectRef {
...
export async function resolveObject(
  ...
  if (!input.name) throw new Error('adt: provide either `objectUri` or `name` (+ optional `type`)');
  const t = normalizeType(input.type);
  ...
  if (strict) {
    ...
    throw new Error(
      `${toolName ? `${toolName}: ` : ''}object '${name}' has no exact match on the backend — mutating tools ` +
  ...
  const hit: AdtObjectSearchHit | undefined = hits[0];
  if (hit) {
    return {
      uri: hit.uri,
...
export function typeLabel(type: string): string {
  const key = type.toUpperCase();
  if (TYPE_MAP[key]) return TYPE_MAP[key]!.label;
  return key;
}
