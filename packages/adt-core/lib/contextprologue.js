import { extractContract, extractDependencyCandidates, rankDependencies } from './abap.js';
...
async function fetchContract(client, cand, signal) {
    try {
        ...
        return { fail: { name: cand.name, reason: `read failed: ${error.message}` } };
    }
}
/**
 ...
 */
export async function buildContextPrologue(client, slice, selfName, options = {}) {
    ...
    while (resolved.length < maxDeps && offset < pool.length) {
        ...
        for (const o of outcomes) {
            if ('ok' in o && resolved.length < maxDeps)
                resolved.push(o.ok);
            else if ('ok' in o)
                unresolved.push({ name: o.ok.name, reason: 'beyond the contextDeps budget' });
            else
                unresolved.push(o.fail);
        }
    }
    ...
    const lines = [];
    lines.push(`"* ═══ dependency context: ${candidates.length} candidate(s) · ${resolved.length} resolved · ` +
