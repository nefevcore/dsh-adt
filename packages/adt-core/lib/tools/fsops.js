...
import { fsCell, fsObjectTypes, fsUnsupportedMessage, renderFsMatrixCard, } from '../fsmatrix.js';
import { typeRegistryRow } from '../typeregistry.js';
import { DESTINATION_PARAM, optStr, text } from './common.js';
export function fsOpsTools(deps, tools) {
    void deps; // engines carry their own deps; the facade only routes
    const owner = (name) => {
        const tool = tools.get(name);
        if (!tool) {
            throw new Error(`adt_object_*: the matrix routes to '${name}' but that tool is not registered — ` +
    ...
    // -------------------------------------------------------------------------
    const frameworkPending = (verb, type, via) => {
        throw new Error(`adt_object_${verb}: ${type} (engine '${via}') is a matrix yes-cell whose engine commit has ` +
    ...
    const readStructured = async (args, exec) => {
        const row = typeRegistryRow(optStr(args.type) ?? '');
        return (await owner('adt_read_structure').execute({ ...args, kind: row.structureKind }, exec));
    };
    const engines = {
        // read engines
        'read:structured': readStructured,
        'read:packageContent': async (args, exec) => owner('adt_package_content').execute({ ...args, packageName: optStr(args.name) ?? optStr(args.packageName) }, exec),
        'read:source': async (args, exec) => owner('adt_read_object').execute(args, exec),
        // read: program text elements (PROG/REPT) — the textelements owner
        // becomes an internal engine (docs/tool-consolidation-plan.md §4).
        'read:textelements': async (args, exec) => owner('adt_read_textelements').execute(args, exec),
        ...
        'edit:structured': async (args, exec) => {
            const row = typeRegistryRow(optStr(args.type) ?? '');
            return (await owner('adt_write_structure').execute({ ...args, kind: row.structureKind }, exec));
        },
        // edit: full-source replace via the OCC write chain.
        'edit:source': async (args, exec) => owner('adt_write_object').execute(args, exec),
        ...
        'write:source': async (args, exec) => owner('adt_create_object').execute(args, exec),
        'write:structured': async (args, exec) => owner('adt_create_object').execute(args, exec),
        'write:fields': async (args, exec) => owner('adt_create_object').execute(args, exec),
        ...
        'delete:delete': async (args, exec) => owner('adt_delete_object').execute(args, exec),
    };
    /** Placeholder router for engines whose commit has not landed. */
    function frameworkPendingRouter(label) {
        return () => {
            throw new Error(`adt_object_*: engine '${label}' has not landed yet — this call refuses instead of degrading. ` +
                'See docs/ddic-fsops-matrix-plan.md §5.');
        };
    }
    async function route(verb, args, exec) {
        ...
        if (!row)
            throw new Error(fsUnsupportedMessage(verb, type));
        ...
        if (part === 'source' && cell.status !== 'yes')
            throw new Error(fsUnsupportedMessage(verb, row.type));
        ...
        if (!engine)
            frameworkPending(verb, row.type, via);
    ...
    return [
        ...
        defineTool({
            ...
            description: 'Create (or, with override:true, overwrite) an ABAP object in one call: type + name + ' +
            ...
            output: {
                ...
                render: (_args, value) => text(`adt_object_write ${value.type} → ${value.created ? 'created' : value.overridden ? 'overridden' : 'written'}${value.activated ? ' + activated' : ''} (via ${value.via ?? 'owner'})`),
            },
            isConcurrencySafe: () => false,
            execute: async (args, exec) => route('write', args, exec),
        }),
        // ------------------------------------------------------------------ read
        defineTool({
            name: 'adt_object_read',
            description: 'Read an ABAP object: source types → source (with OCC snapshot), structured types ' +
                ...
                render: (_args, value) => {
                    ...
                    if (value.verb === 'read' && value.elements !== undefined) {
                        ...
                        const lines = [
                            ...
                            ...value.elements.map((e) => `- [${e.id}] ${e.key} = ${e.entry}${e.length ? ` (max ${e.length})` : ''}`),
                        ];
                        if (value.note)
                            lines.push(`Note: ${String(value.note)}`);
            ...
            execute: async (args, exec) => {
                ...
                if (optStr(args.name) === undefined) {
                    return { matrixCard: renderFsMatrixCard() };
                }
                return route('read', args, exec);
            },
        }),
        // ------------------------------------------------------------------ edit
        defineTool({
            name: 'adt_object_edit',
            description: 'Modify an EXISTING object (read-then-patch): source types take full new source ' +
            ...
            output: {
                ...
                render: (_args, value) => text(`adt_object_edit ${value.type} → patched (via ${value.via ?? 'owner'})`),
            },
            isConcurrencySafe: () => false,
            execute: async (args, exec) => {
                ...
                const hasPatch = optStr(argsRecord.source) !== undefined || optStr(argsRecord.sourceFile) !== undefined ||
                ...
                if (!hasPatch) {
                    throw new Error('adt_object_edit: nothing to patch — pass `source`/`sourceFile` (source types) or the ' +
                        'structured patch fields (properties/fixedValues/labels/messages/description).');
                }
                return route('edit', argsRecord, exec);
            },
        }),
        // ---------------------------------------------------------------- delete
        defineTool({
            name: 'adt_object_delete',
            description: 'Delete an ABAP object (irreversible). The delete verb is independent of the write/edit ' +
            ...
            output: {
                ...
                render: (_args, value) => text(`adt_object_delete ${value.type} → ${value.deleted === true ? 'deleted' : 'done'}`),
            },
            isConcurrencySafe: () => false,
            execute: async (args, exec) => route('delete', args, exec),
        }),
    ];
}
