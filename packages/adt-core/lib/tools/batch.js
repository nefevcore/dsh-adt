 ...
 */
const CONTROL_CHARS = /[\u0000-\u001F\u007F]/;
function assertNoControlChars(label, value, index, toolName) {
    if (CONTROL_CHARS.test(value)) {
        throw new Error(`${toolName}: requests[${index}] ${label} contains control characters (CR/LF/NUL…) — header injection into the ` +
...
function validateBatchPart(part, index, allowWrites, toolName) {
    ...
    if (part.accept !== undefined)
        assertNoControlChars('accept', part.accept, index, toolName);
    if (part.contentType !== undefined)
        assertNoControlChars('contentType', part.contentType, index, toolName);
...
export function batchTools(deps, ctx) {
    ...
    const batch = defineTool({
        name: 'adt_batch',
        description: 'Execute several ADT requests in ONE HTTP round-trip (protocol-level $batch: multipart embedded ' +
        ...
        execute: async (args, exec) => {
            ...
            const normalized = parts.map((part) => ({
                method: (part.method ?? 'GET').toUpperCase(),
                path: part.path,
            ...
            try {
                ...
                if (isAdtServiceUnavailable(error)) {
                    throw new Error(`$batch is not available on destination '${entry.config.name}' (HTTP ${error.status}) — this backend does ` +
    ...
    const exportObjects = defineTool({
        ...
        description: 'Export the sources of ABAP objects (CLAS, INTF, PROG, FUGR/FUNC, DDLS, TABL, STRU, DOMA, DTEL, TTYP, ' +
        ...
        execute: async (args, exec) => {
            ...
            const fs = ctx.get('fs');
            if (!fs)
                throw new Error('adt_export_objects requires a host filesystem service');
            ...
            let failed = 0;
            for (const o of args.objects.slice(0, clamp.value)) {
                try {
                    ...
                    failed++;
                    files.push({ name: o.name, path: `FAILED: ${error.message}` });
                }
            }
            return {
                targetDir,
