...
export function objectTools(deps) {
    const { registry, ledger } = deps;
    const createObject = defineTool({
        name: 'adt_create_object',
        description: 'Create a new ABAP development object: class (CLAS), interface (INTF), program (PROG), CDS view (DDLS), ' +
        ...
        parameters: {
            ...
            source: {
                type: 'string',
                description: 'Source types (PROG/CLAS/INTF/FUNC/DDLS/DCLS/DDLX/BDEF/SRVD): full source written via the locked PUT chain after the create. ' +
                    'Empty string = placeholder (create only).',
            },
            fields: {
                type: 'array',
                description: 'TABL only: field list for the one-step DDL flow. Each: {name, type (builtin code or data element name), ' +
        ...
        execute: async (args, exec) => {
            ...
            if (String(args.type).toUpperCase() === 'TABL' && Array.isArray(args.fields)) {
                ...
                if (rawFields.length === 0) {
                    throw new Error('adt_create_object: `fields` must contain at least one field ' +
    ...
    const deleteObject = defineTool({
        ...
        description: 'Delete an ABAP development object (modern deletion service, legacy `_action` fallback). Irreversible — ' +
        ...
        parameters: {
            ...
            transport: {
                type: 'string',
                description: 'Transport request number the deletion is recorded into. When omitted the backend decides on lock ' +
