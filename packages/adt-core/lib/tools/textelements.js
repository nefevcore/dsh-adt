...
export function textElementTools(deps) {
    const { registry } = deps;
    return [
        defineTool({
            name: 'adt_read_textelements',
            description: 'Read the text elements of an ABAP program (PROG/REPT, SE38 selection-screen texts): text symbols (I, ' +
            ...
            execute: async (args, exec) => {
                ...
                const note = result.counts.symbols + result.counts.selections + result.counts.headings === 0
