    ...
    return [
        defineTool({
            ...
            parameters: {
                ...
                associations: {
                    type: 'boolean',
                    description: 'List the CDS associations of the entity (name+kind, DDLS): name, target and cardinality of each. ' +
                        'The JOIN-alternative for CDS views — follow one with the `association` parameter.',
                },
                association: {
                    type: 'string',
                    description: "Follow ONE CDS association of the entity (e.g. '_Bookings') and return the associated rows — " +
            ...
            execute: async (args, exec) => {
                    ...
                    try {
                        ...
                        if (error instanceof AdtError && error.status === 400) {
                            ...
                            if (/[~.]\s*is expected|not allowed here/i.test(msg) || /~/.test(msg)) {
                                ...
                                throw new Error(`SQL rejected by the backend (HTTP 400): ${msg}. Common cause: ${swap}. ` +
                            ...
                            if (/DDL source.*could not be read/i.test(msg)) {
                                throw new Error(`Association navigation is not reachable on this backend (HTTP 400): ${msg}. ` +
                ...
                if (sql) {
                    ...
                    if (compiled) {
                        const result = await compileAndRun(linted.sql, (fetchSql, fetchOpts) => run(() => entry.client.runSqlQuery(fetchSql, { top: fetchOpts.top, signal: fetchOpts.signal ?? exec.signal })), { length: paging.length, offset: paging.offset, signal: exec.signal });
                ...
                if (wantAssociations || followAssociation) {
                    ...
                    if (followAssociation) {
                        const paging = resolveRowWindow(args, notes);
                        const result = await run(() => entry.client.followCdsAssociation(name, followAssociation, { top: paging.fetchTop, signal: exec.signal }));
                        const rows = pageRows(result.rows, paging.offset, paging.length, notes);
