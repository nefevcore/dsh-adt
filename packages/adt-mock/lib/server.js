...
import { OBJECTS, PACKAGES } from './data.js';
...
export function createMockAdtServer(options = {}) {
    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            res.end(errorXml(`Internal mock error: ${error.message}`));
        }
    });
    return {
        server,
...
async function handle(req, res, state, opts) {
  ...
  ${runs}
    ...
    if (atcResultMatch && req.method === 'GET') {
        ...
        const files = targets
            ...
            const errors = (o.atcFindings ?? [])
                .map((f) => `<error message="${xmlEscape(f.message)}" source="${f.check}" line="${f.line ?? 1}" severity="${f.severity === 'CRITICAL' ? 'error' : f.severity.toLowerCase()}"/>`)
                .join('\n    ');
    ...
    ${errors}
            ...
            .join('\n  ');
        res.end(adtXml(`<checkstyle version="1.0">
  ${files}
</checkstyle>`));
        return;
    }
