...
function dataPreviewXml(entity, query = '', rows = 2) {
    ...
    const texts = Array.from({ length: count }, (_, i) => (i % 2 === 1 ? '<dataPreview:data/>' : `<dataPreview:data>Row ${i + 1}</dataPreview:data>`)).join('');
...
const strictStatefulSessions = new Set();
/**
...
async function strictGatesFull(req, res, state, ctx) {
    ...
    if (sessionCookie && isStateful)
        strictStatefulSessions.add(sessionCookie);
...
function hDataPreviewCdsAssociations({ res, url }) {
    ...
    if (action === 'associationlist') {
        res.setHeader('Content-Type', 'application/vnd.sap.adt.datapreview.table.v1+xml');
        res.end(adtXml(`<assoc:associationlist xmlns:assoc="http://www.sap.com/adt/datapreview">\n` +
...
async function hDeletionService({ req, res, state }) {
    const body = await readBody(req);
...
function hStrictLockParamMissing({ res, url }) {
    res.statusCode = 400;
    res.setHeader('Content-Type', 'application/xml');
    res.end(errorXml(`Parameter accessMode could not be found (strict profile: LOCK requires ?_action=LOCK&accessMode=MODIFY, got accessMode='${url.searchParams.get('accessMode') ?? ''}')`, 'ExceptionParameterNotFound'));
}
