 ...
 */
async function strictGatesFull(
  req: IncomingMessage,
  res: ServerResponse,
  state: MockState,
  ctx: { path: string; url: URL },
): Promise<{ refused: boolean; req: IncomingMessage }> {
  ...
  if (req.method === 'POST' && (ctx.path === '/ddic/tables' || ctx.path === '/ddic/structures'
    ...
    if (/<(adtcore:object|adtcore:objectReference)[\s>]/.test(body)) {
      ...
      res.setHeader('Content-Type', 'application/xml');
      res.end(errorXml(
        `The request contains an invalid body (strict profile: this collection expects its own namespaced root element, not the generic ${/objectReference/.test(body) ? 'adtcore:objectReference' : 'adtcore:object'} form)`,
 ...
 */
function replayReq(req: IncomingMessage, body: string): IncomingMessage {
  const buffer = Buffer.from(body, 'utf8');
  let delivered = false;
  const stub = req as IncomingMessage & { [Symbol.asyncIterator](): AsyncGenerator<Buffer> };
  stub[Symbol.asyncIterator] = async function* (): AsyncGenerator<Buffer> {
    if (delivered) return;
    delivered = true;
...
function hAtcResultDetail({ res, state, path }: RequestCtx): void {
  ...
  res.setHeader('Content-Type', 'application/xml');
  const priorityOf = (severity: string): number =>
    severity === 'CRITICAL' ? 1 : severity === 'ERROR' ? 2 : severity === 'WARNING' ? 3 : 4;
  ...
  for (const o of targets) {
    for (const f of o.atcFindings ?? []) {
      counts[`p${priorityOf(f.severity)}` as keyof typeof counts]++;
    }
  }
  res.end(
    adtXml(
