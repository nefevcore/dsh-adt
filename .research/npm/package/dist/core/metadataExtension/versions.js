...
exports.getMetadataExtensionVersions = getMetadataExtensionVersions;
exports.getMetadataExtensionVersionSource = getMetadataExtensionVersionSource;
const internalUtils_1 = require("../../utils/internalUtils");
const timeouts_1 = require("../../utils/timeouts");
const versions_1 = require("../shared/versions");
const ACCEPT_VERSION_FEED = 'application/atom+xml;type=feed';
// candidate URI — probe-verify on trial
async function getMetadataExtensionVersions(connection, config) {
    if (!config.name)
    ...
    try {
        ...
        (0, versions_1.throwVersionsError)(e, `metadata extension ${config.name}`);
    }
}
async function getMetadataExtensionVersionSource(connection, contentUri) {
    try {
        const res = await connection.makeAdtRequest({
            url: contentUri,
            method: 'GET',
